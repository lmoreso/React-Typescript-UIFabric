    ...
    return (
        [
            ...
            { title: strings.field_Flag, field: "banderaUrl", width: 35, isImage: true },
            {
                ...
                txtHeaderTooltip: strings.click_ToSeeCountryInGoogleMaps,
            },
            {
...
/*     // Pruebas
    console.log('Buscando en Wiki', );
    try {
        const url = `https://en.wikipedia.org/w/api.php?action=query&titles=Belgrade&prop=extracts&format=json&exintro=1&origin=*`;
        let respuesta = await fetch(url);  // , { mode: 'no-cors', credentials: 'omit',  }
        console.log('Respuesta Wiki', respuesta);
    } catch (error) {
        console.log('error', error);
    }
    console.log('Buscando en Restcountries', );      
    try {
        const url = URL_COUNTRIES;
        let respuesta = await fetch(url);
        console.log('Respuesta Restcountries', respuesta);
    } catch (error) {
        console.log('error', error);
    }
 */
